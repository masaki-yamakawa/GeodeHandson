...
EmbeddedモードのCUIチャットアプリ作成では、Javaアプリケーションの中にデータの管理を行うGeodeを組み込む方法を学びました。
...
geode.handson.cui.ChatMessageListener | （EmbeddedモードのCUIチャットアプリ作成と共有）
/resources/gemfire.properties         | （EmbeddedモードのCUIチャットアプリ作成と共有）
/resources/clientcache.xml            | クライアントキャッシュ（リージョン）を設定するためのファイルです。<br>デフォルトではEmbeddedモードで使用したcache.xmlが使用されてしまうため、gemfire.propertiesのcache-xml-fileを変更するかAPIで変更する必要があります。
...
gfshを起動したら以下のコマンドを使用してロケーター、キャッシュサーバーの順に起動します。
...
クラスターの状態を確認するにはgfshでlist membersコマンドを使用するか、Web管理コンソールのPulseを使用します。
...
クライアントキャッシュを作成する場合も、キャッシュを作成する場合と同様の感覚で行うことが出来ます。
...
Propertiesへcache-xml-fileを指定することで、デフォルトのcache.xmlの値を上書きして、今回使用する設定ファイル名へ変更しています。
...
他にもregisterInterestを複数回実行し、特定のキーのイベントのみを取得したり、registerInterestRegexでキーの正規表現を使用することもできます。
